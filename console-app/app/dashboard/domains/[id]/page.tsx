    ...
    <div className="domainspage" style={{ maxWidth: 920 }}>
      ...
      {isOnboarding && (
        <div style={{
          ...
          gap: 16,
          flexWrap: 'wrap',
          boxShadow: '0 2px 10px rgba(8, 102, 245, 0.04)'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, color: 'var(--d-ink)', fontSize: '1rem' }}>
                {step === 0
                  ? (detected ? `Step 1 of 3: Verify Ownership on ${PROVIDERS[detected]?.label}` : 'Step 1 of 3: Verify Domain Ownership')
                  : step === 1
                  ? 'Step 2 of 3: Configure DNS Records'
                  : '🎉 Step 3 of 3: Business Email Ready!'}
              </div>
              <div style={{ color: 'var(--d-muted)', fontSize: '0.84rem', marginTop: 2 }}>
                {step === 0
                  ? (detected ? `We detected your DNS is on ${PROVIDERS[detected]?.label}. Verify domain ownership to proceed.` : `Verify ownership of ${domain.domain} before configuring DNS.`)
                  : step === 1
                  ? `Publish MX & DKIM records for ${domain.domain} to enable email routing.`
                  : `Your personalised business email on ${domain.domain} is active and ready to use!`}
              </div>
            </div>
          </div>
          <span style={{
            ...
            {step === 0 ? 'Step 1/3' : step === 1 ? 'Step 2/3' : 'Completed'}
          </span>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="d-addcard" style={{ textAlign: 'center', marginBottom: 20, padding: '36px 24px' }}>
          ...
          <p style={{ color: 'var(--d-muted)', fontSize: '0.92rem', marginBottom: '1.75rem' }}>
            Domain ownership verified, DNS configured, and your personalised email is active on <strong>{domain.domain}</strong>.
          </p>
