...
# ─── nginx front for JMAP ────────────────────────────────────────────────────
# Only the mail.* vhost is this script's business. The console / webmail vhosts are
# written by their own deploy (migrate-from-do.sh step 3).
echo ""
...
echo "  sudo certbot --nginx -d ${DOMAIN} -d mail.${DOMAIN} && sudo bash \$0"
