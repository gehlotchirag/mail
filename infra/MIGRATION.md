...
This copies both Postgres databases, streams the Flux on-disk state over, and deploys
the console app under pm2. Mailbox *contents* move through the IMAP migration workers,
not this script — on AWS the message store is Postgres + S3, not a local directory.
...
- 100 GB of mail blobs in S3. Versioning is **on** with no lifecycle rule, so deleted
  and overwritten objects keep billing. At 500 GB this line is ~$13, not $2.80.
